! function(window) {
	function Element() {
		this.initialize = function() {
			this.elementType = "element", this.serializedProperties = ["elementType"], this.propertiesStack = [], this._id = "" + Math.ceil(Math.random() * (new Date).getTime())
	...
	CanvasRenderingContext2D.prototype.JTopoRoundRect = function(a, b, c, d, e) {
		...
		MouseCursor: {
			...
			move: "move",
			open_hand: "url(./images/cur/openhand.cur) 8 8, default", //鼠标 手掌张开样式
			closed_hand: "url(./images/cur/closedhand.cur) 8 8, default" //鼠标 手掌蜷起样式
		},
		createStageFromJson: function(jsonObj, canvas) {
			eval("var jsonObj = " + jsonObj);
...
function(a) {
	...
	function c(c) {
		...
		q.forEach(function(a) {
			...
			function wuxiandiguidiaoyong() {
				if(n.frames == 0){
					setTimeout(wuxiandiguidiaoyong, 100)
				}else if(n.frames < 0){
					n.repaint(), setTimeout(wuxiandiguidiaoyong, 1e3 / -n.frames)
				}else{
					n.repaint(), setTimeout(wuxiandiguidiaoyong, 1e3 / n.frames)
				}
