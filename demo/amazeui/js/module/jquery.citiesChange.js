...
(function(root, factory) {
	...
	$.initProv = function(prov, city, defaultProv, defaultCity) {
		var provEl = $(prov),
			cityEl = $(city),
			hasDefaultProv = (typeof(defaultProv) != 'undefined'),
			provHtml = '';
		for (var i = 0; i < $._cityInfo.length; i++) {
			provHtml += '<option index="' + i + '" value="' + $._cityInfo[i].n + '"' + ((hasDefaultProv && $._cityInfo[i].n == defaultProv) ? ' selected="selected"' : '') + '>' + $._cityInfo[i].n + '</option>';
		}

		provEl.html(provHtml).change(function() {
			$.initCities(provEl, cityEl, defaultCity);
		}).chosen({
			no_results_text: '没有找到这个城市!',
			width: '100%'
		});
		$.initCities(provEl, cityEl, defaultCity);
	};
	$.initCities = function(provEl, cityEl, defaultCity) {
		var hasDefaultCity = (typeof(defaultCity) != 'undefined'),
			provIndex = $(provEl).find("option:selected").attr("index");

		if (provIndex != '' && parseInt(provIndex) >= 0) {
			var cities = $._cityInfo[parseInt(provIndex)].c;
			var cityHtml = '';
			for (var i = 0; i < cities.length; i++) {
				cityHtml += '<option data-index="' + i + '" value="' + cities[i] + '"' + ((hasDefaultCity && cities[i] == defaultCity) ? ' selected="selected"' : '') + '>' + cities[i] + '</option>';
			}

			cityEl.html(cityHtml).chosen({
				disable_search_threshold: 5,
				no_results_text: '没有找到这个区县!',
				width: '100%'
